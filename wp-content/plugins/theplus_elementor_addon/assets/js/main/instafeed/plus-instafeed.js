(function ($) {
	...
	var InstagramFeedList = function( $scope, $ ) {
		...
		var InstagramFeedList = $scope.find('.theplus-instagram-feed'),
			...
			likes = (options.data('likes') === 'yes') ? '<span class="theplus-insta-post-likes"> <i class="fa fa-heart-o" aria-hidden="true"></i> {{likes}}</span>' : '',
			comments = (options.data('comments') === 'yes') ? '<span class="theplus-insta-post-comments"><i class="fa fa-comment-o" aria-hidden="true"></i> {{comments}}</span>' : '',
						...
						success: function(data){
							...
							if(data.is_private){
								...
								for(var i = 0; i < max; i++) {
									...
									likes = (options.data('likes') === 'yes') ? '<span class="theplus-insta-post-likes"> <i class="fa fa-heart-o" aria-hidden="true"></i> '+imgs[i].node.edge_media_preview_like.count+'</span>' : '';
									
									comments = (options.data('comments') === 'yes') ? '<span class="theplus-insta-post-comments"><i class="fa fa-comment-o" aria-hidden="true"></i> '+imgs[i].node.edge_media_to_comment.count+'</span>' : '';
