...
class ThePlus_Circle_Menu extends Widget_Base {
    ...
    protected function _register_controls() {
	 ...
	 protected function render() {
		...
		$show_bg_overlay_color_value = ($settings['show_bg_overlay_color']=='yes') ? 'data-overlay-color="'.$settings['show_bg_overlay_color_value'].'"' : '';
		...
		$circle_menu ='<div class="plus-circle-menu-wrapper '.esc_attr($uid).' layout-'.esc_attr($icon_layout_open).' '.esc_attr($show_scroll_window_offset).' '.$animated_class.' " '.$animation_attr.' data-uid='.esc_attr($uid).' '.$scroll_top_offset_value.' '.$show_bg_overlay_color_value.'>';
				...
				$main_toggle_click .= '<li class="plus-circle-main-menu-list plus-circle-menu-list '.esc_attr($toggle_open_icon_style).'">';
					if(!empty($loop_icon_main_style) && $loop_image_main_icon == 'icon' && $loop_icon_main_style=='font_awesome'){
						...
						$icons_main='<i class="'.$loop_icons_main_mind.' toggle-icon-wrap" ></i>';
							...
							$img = wp_get_attachment_image_src($loop_select_main_image,$settings['loop_select_main_image_thumbnail_size']);
							$loop_select_main_image_Src = $img[0];
						$icons_main= '<img src="'.$loop_select_main_image_Src.'" class="toggle-icon-wrap" />';
					}else{
						$icons_main='';
					}
					$close_toggle='';
					...
					$main_toggle_click .= '<a class="main_menu_icon" style="cursor:pointer;">'.$icons_main.$close_toggle.'</a>';
							...
							if(!empty($item['loop_image_icon'])){								
									...
									if(isset($item['loop_image_icon']) && $item['loop_image_icon'] == 'image'){
										...
										if($icon_layout_open=='straight' && $icon_layout_straight_style=='menu-style-2'){
											...
											$circle_menu .='<a '.$nolink.' class="menu_icon"><img class="img" src='.esc_url($loop_imgSrc).' />'.$tooltip_title.'</a>';
										}
										...
										if($icon_layout_open=='straight' && $icon_layout_straight_style=='menu-style-2'){
											...
											$circle_menu .= '<a '.$nolink.' class="menu_icon" ><i class=" '.esc_attr($icons).' " ></i>'.$tooltip_title.'</a>';
										}
									}
							}
