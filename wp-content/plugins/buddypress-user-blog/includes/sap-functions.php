...
function sap_template_blog() {
    ...
    sap_load_template( 'sap-blog-page', array(
            ...
            'max_num_pages' => isset($max_num_pages) ? $max_num_pages : $wp_query->max_num_pages,
            'paged' => $paged,
            'sort' => $sort,
    ) );
...
function sap_get_bookmarked_posts( $args = array() ) {
    ...
    if ( is_array( $bookmarked_posts ) && count( $bookmarked_posts ) > 0 && $bookmarks_qv['number'] > 0 ) {
...
function sap_save_post() {
    ...
    // hashtagger plugin generate convert a #hashtag from post content into a Post Tag
    if ( isset( $pp_hashtagger ) ) {
        wp_set_post_tags( $pid, $pp_hashtagger->get_hashtags_from_content( strip_tags( $post_content[ 'value' ] ) ) );
    }
...
function mb_profile_menu_tabs(){
    ...
    $view_member_type = bp_get_member_type($user_id);
    $logged_member_type = bp_get_member_type(get_current_user_id());

    $member_roles = bp_get_member_types();

    unset($member_roles['administrator']);
    unset($member_roles['teaching-assistant']);

    if ( in_array($logged_member_type, $member_roles)) {
        if(bp_displayed_user_id() != get_current_user_id() && $view_member_type != 'administrator'  && $view_member_type !=''){ 
            echo '<style>#blog-personal-li{display:none;} .sap-category-widget, .sap-tags-widget{display:none;}</style>';
            }   
    }  }

if ( ! is_admin()) {
    add_action('bp_setup_nav', 'mb_profile_menu_tabs');
} 