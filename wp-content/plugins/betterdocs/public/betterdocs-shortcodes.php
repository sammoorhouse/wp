...
function betterdocs_category_grid( $atts, $content = null ) {
	...
	if ( $taxonomy_objects && ! is_wp_error( $taxonomy_objects ) ) :
		...
		if(!is_singular('docs') && !is_tax('doc_category') && !is_tax('doc_tag')){
			if(isset($get_args['masonry']) && $get_args['masonry'] == true){
				$class[] = 'layout-masonry';
			}elseif($masonry_layout == 1 && $nested_subcategory != 1){
				$class[] = 'layout-masonry';
			}else{
				$class[] = 'layout-flex';
			}
					...
					<div class="<?php echo esc_attr($title_class) ?>">
						<div class="docs-cat-title-inner">
							...
							if($post_count == 1 && $get_args['post_counter'] == true){
								echo '<div class="docs-item-count"><span>'.$term->count.'</span></div>';
							}
							?>
						</div>
					</div>
					<div class="docs-item-container">
						<?php 
							...
							if($nested_subcategory == 1 || $get_args['nested_subcategory'] == true) {
								...
								$sub_taxonomies = array('parent' => $term->term_id);
...
function betterdocs_feedback_form_submit() {
	...
	//response messages
	$missing_name = __('Please enter your name.','betterdocs');
	$email_invalid   = __('Enter a valid email address.','betterdocs');
	$missing_message = __('Please write your message.','betterdocs');
	$message_unsent  = __('Message was not sent. Try Again.','betterdocs');
	$message_sent    = __('Thanks! Your message has been sent.','betterdocs');
	...
	$subject = "Feedback message from ".get_bloginfo('name');
	$headers = 'From: '. $email . "\r\n" .
	'Reply-To: ' . $email . "\r\n";
