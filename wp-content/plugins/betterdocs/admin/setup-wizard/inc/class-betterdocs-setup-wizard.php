...
if( ! class_exists( 'BetterDocsSetupWizard' ) ){
    class BetterDocsSetupWizard {
        ...
        // add admin page
        public static function existing_plugins_url($plugins) {
            $url = '';
            if ($plugins === 'wedocs') {
                $url = 'wedocs/wedocs.php';
            }
            if ($plugins === 'bsf-docs') {
                $url = 'bsf-docs/bsf-docs.php';
            }
            if ($plugins === 'documentor-lite') {
                $url = 'documentor-lite/documentor-lite.php';
            }
            return $url;
        }
        ...
        public static function better_docs_quick_setup_wizard_data_save(){
            ...
            if(isset($_POST['activate_plugin'])){
                setcookie("activate_plugin", $_POST['activate_plugin'], time()+3600);
                $existing_plugins_url = self::existing_plugins_url($_POST['activate_plugin']);
                setcookie("activate_plugin_url", $existing_plugins_url, time()+3600);
                deactivate_plugins( $existing_plugins_url );
            }
