(function ($) {
	...
	$(document).ready(function () {
		...
		if (qVars != undefined) {
			if (qVars.indexOf('betterdocs-settings') >= 0) {
				...
				$('.betterdocs-settings-menu li[data-tab="' + cSettingsTab[1] + '"]').trigger('click');
			}
		}
	...
	$.betterdocsAdmin.bindEvents = function () {
		...
		$('body').on('click', '.betterdocs-adv-checkbox-wrap label, .betterdocs-stats-tease', function (e) {
		 ...
		 * Group Field Events
		 */
		$('.betterdocs-group-field .betterdocs-group-field-title').on('click', function (e) {
			e.preventDefault();
			if ($(e.srcElement).hasClass('betterdocs-group-field-title')) {
				$.betterdocsAdmin.groupToggle(this);
			}
		});
		$('.betterdocs-group-field .betterdocs-group-clone').on('click', function () {
			$.betterdocsAdmin.cloneGroup(this);
		});
		$('body').on('click', '.betterdocs-group-field .betterdocs-group-remove', function () {
		 ...
		 * Media Field
		 */
		$('.betterdocs-media-field-wrapper .betterdocs-media-upload-button').on('click', function (e) {
			e.preventDefault();
			$.betterdocsAdmin.initMediaField(this);
		});
		$('.betterdocs-media-field-wrapper .betterdocs-media-remove-button').on('click', function (e) {
			e.preventDefault();
	...
	$.betterdocsAdmin.initializeFields = function () {
		...
		if ($('.betterdocs-meta-field, .betterdocs-settings-field').length > 0) {
			$('.betterdocs-meta-field, .betterdocs-settings-field').map(function (iterator, item) {
				...
				$('.betterdocs-colorpicker-field').each(function () {
					...
					$(this).wpColorPicker({
						change: function (event, ui) {
							...
							$(element).parents('.wp-picker-container').find('input.betterdocs-colorpicker-field').val(color).trigger('change');
						}
	...
	$.betterdocsAdmin.innerTab = function(){
		...
		$('.betterdocs-section-inner-tab').each(function( i, item ){
			$(item).find('ul').on('click', 'li', function( e ){
				var target = e.currentTarget.dataset.target;
				$( this ).addClass('betterdocs-active').siblings().removeClass('betterdocs-active');
	...
	$.betterdocsAdmin.groupField = function () {
		...
		fields.each(function () {
			...
			groups.each(function () {
				var groupContent = $(this).find('.betterdocs-group-field-title:not(.open)').next();
			...
			$this.find('.betterdocs-group-field-add').on('click', function (e) {
				...
				clone.find('tr.betterdocs-field[id*=' + fieldId + ']').each(function () {
	...
	$.betterdocsAdmin.tabChanger = function (buttonName) {
		...
		$('.betterdocs-metatab-menu li[data-tabid="' + tabID + '"]').trigger('click');
		$('.betterdocs-builder-tab-menu li[data-tabid="' + tabID + '"]').trigger('click');
	};

	$.betterdocsAdmin.toggleFields = function () {
		$(".betterdocs-meta-field, .betterdocs-settings-field").on('change', function (e) {
	...
	$.betterdocsAdmin.checkDependencies = function (variable) {
		...
		if (current.hasClass('betterdocs-theme-selected')) {
			var currentTheme = current.parents('.betterdocs-theme-control-wrapper').data('name');
		...
		if (betterdocsAdminConfig.toggleFields.hasOwnProperty(id)) {
			var canShow = betterdocsAdminConfig.toggleFields[id].hasOwnProperty(value);
			var canHide = true;
			if (betterdocsAdminConfig.hideFields[id]) {
				var canHide = betterdocsAdminConfig.hideFields[id].hasOwnProperty(value);
			}

			if (betterdocsAdminConfig.toggleFields.hasOwnProperty(id) && canHide) {
				$.each(betterdocsAdminConfig.toggleFields[id], function (key, array) {
					$.betterdocsAdmin.toggle(array.fields, 'hide', '#betterdocs-meta-', '', mainid);
					$.betterdocsAdmin.toggle(array.sections, 'hide', '#betterdocs-settings-', '', mainid);
				})
			}

			if (canShow) {
				$.betterdocsAdmin.toggle(betterdocsAdminConfig.toggleFields[id][value].fields, 'show', '#betterdocs-meta-', '', mainid);
				$.betterdocsAdmin.toggle(betterdocsAdminConfig.toggleFields[id][value].sections, 'show', '#betterdocs-settings-', '', mainid);
			}
		}
		...
		if (betterdocsAdminConfig.hideFields.hasOwnProperty(id)) {
			...
			if (hideFields.hasOwnProperty(value)) {
				$.betterdocsAdmin.toggle(hideFields[value].fields, 'hide', '#betterdocs-meta-', '', mainid);
				$.betterdocsAdmin.toggle(hideFields[value].sections, 'hide', '#betterdocs-settings-', '', mainid);
			}
		}
	...
	$.betterdocsAdmin.removeGroup = function (button) {
		var groupId = $(button).parents('.betterdocs-group-field').attr('data-id'),
			group = $(button).parents('.betterdocs-group-field[data-id="' + groupId + '"]'),
			parent = group.parent();
	...
	$.betterdocsAdmin.cloneGroup = function (button) {
		var groupId = $(button).parents('.betterdocs-group-field').attr('data-id'),
			group = $(button).parents('.betterdocs-group-field[data-id="' + groupId + '"]'),
			clone = $(group.clone()),
			lastGroup = $(button).parents('.betterdocs-group-fields-wrapper').find('.betterdocs-group-field:last'),
			parent = group.parent(),
	...
	$.betterdocsAdmin.resetFieldIds = function (groups) {
		...
		groups.map(function (iterator, item) {
			...
			var item = $(item),
				...
				groupInfo = item.find('.betterdocs-group-field-info').data('info'),
			...
			table_row.each(function (i, child) {
				...
				var child = $($(child)[0]),
					childInput = child.find('[name*="betterdocs_meta_' + fieldName + '"]'),
	...
	$.betterdocsAdmin.initMediaField = function (button) {
		...
		frame.on('select', function () {
			 ...
			 */
			imgContainer.addClass('betterdocs-has-thumb').append('<img src="' + attachment.url + '" alt="" style="max-width:100%;"/>');
		...
		$.ajax({
			...
			success: function (res) {
				...
				if (res === 'success') {
					...
					$('.betterdocs-save-now').removeClass("betterdocs-save-now");
				} else {
					swal({
