...
if (!class_exists('ProgressAllyiSDK')) {
if (!function_exists('xmlrpc_encode_entitites')) {
    include("xmlrpc-3.0/lib/xmlrpc.inc");
}

class ProgressAllyiSDKException extends Exception
{
}

class ProgressAllyiSDK
{
    ...
    public function cfgCon($name, $key = "", $dbOn = "on")
    {
        ...
        try {
            ...
            if (strpos($connected, 'ERROR') !== FALSE) {
                throw new ProgressAllyiSDKException($connected);
            }

        } catch (ProgressAllyiSDKException $e) {
            throw new ProgressAllyiSDKException($e->getMessage());
        }
    ...
    public function vendorCon($name, $user, $pass, $key = "", $dbOn = "on")
    {
        ...
        if ($key != "" && $key != "on" && $key != "off" && $key != 'kill' && $key != 'throw') {
            ...
            if (!empty($details[$name])) {
                if ($details[$name][2] == "i") {
                    ...
                    throw new ProgressAllyiSDKException("Invalid application name: \"" . $name . "\"");
                }
            } else {
                throw new ProgressAllyiSDKException("Application Does Not Exist: \"" . $name . "\"");
            }
            $this->key = $details[$name][3];
        }
        ...
        try {
            ...
            throw new ProgressAllyiSDKException("Connection Failed");
        }
        return TRUE;
    }
    ...
    public function methodCaller($service, $callArray)
    {
        ...
        if (!$result->faultCode()) {
            ...
            if ($this->debug == "kill") {
                ...
                throw new ProgressAllyiSDKException($result->faultString(), $result->faultCode());
