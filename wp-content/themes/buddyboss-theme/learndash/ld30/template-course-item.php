...
$members_arr         = learndash_get_users_for_course( get_the_ID(), array(), false );
...
if ( $members ) {
	foreach ( $members as $member ) {
		$member = get_userdata( (int) $member );
...
<li class="bb-course-item-wrap">
    ...
    <div class="bb-cover-list-item <?php echo $class; ?>">
            ...
            <a title="<?php the_title_attribute(); ?>" href="<?php the_permalink(); ?>" class="bb-cover-wrap">
				...
				if ( is_user_logged_in() && isset( $has_access ) && $has_access ) {
					...
					if ( ( $course_pricing['type'] === 'open' && $progress['percentage'] === 0 ) || ( $course_pricing['type'] !== 'open' && $has_access && $progress['percentage'] === 0 ) ) {
						...
						echo '<div class="ld-status ld-status-progress ld-primary-background">' . __( 'Start ',
								'buddyboss-theme' ) . __( LearnDash_Custom_Label::get_label( 'course' ),
								'buddyboss-theme' ) . '</div>';
        ...
        <div class="bb-card-course-details">
			...
			$total_lessons = $lessons_count > 1 ? sprintf( __( '%s ',
					'buddyboss-theme' ) . __( LearnDash_Custom_Label::get_label( 'lessons' ), 'buddyboss-theme' ),
				$lessons_count ) : sprintf( __( '%s ',
					'buddyboss-theme' ) . __( LearnDash_Custom_Label::get_label( 'lesson' ), 'buddyboss-theme' ),
				$lessons_count );

			if ( $lessons_count > 0 ) {
				echo '<div class="course-lesson-count">' . $total_lessons . '</div>';
			} else {
				echo '<div class="course-lesson-count">' . __( '0 ',
						'buddyboss-theme' ) . __( LearnDash_Custom_Label::get_label( 'lessons' ),
						'buddyboss-theme' ) . '</div>';
			}
			?>
