...
if ( !function_exists( 'buddyboss_theme_ld_style' ) ) {

	function buddyboss_theme_ld_style() {

        if ( class_exists( 'SFWD_LMS' ) ) {
            $is_admin_bar = is_admin_bar_showing() ? 32 : 0;
            $header_height = buddyboss_theme_get_option( 'header_height' ) ? buddyboss_theme_get_option( 'header_height' ) : 0;
            $header_sum = $header_height + $is_admin_bar;

            echo '<style id="bb-learndash-style">';
            if ( buddyboss_is_learndash_inner() && is_admin_bar_showing() ) { ?>
                html{margin-top:0 !important;}
                header.site-header{margin-top:32px;}
                .bb-sfwd-aside.sticky-header .site-content,.bb-sfwd-aside.bb-buddypanel.sticky-header .site-content{padding-top:<?php echo $header_sum; ?>px;}
                .bb-sfwd-aside .buddypanel{padding-top:32px;}
                @media screen and (max-width: 782px) {
                    header.site-header{margin-top:46px;}
                    .bb-sfwd-aside.sticky-header .site-content,.bb-sfwd-aside.bb-buddypanel.sticky-header .site-content{padding-top:<?php echo $header_sum + 14; ?>px;}
                    .bb-sfwd-aside .buddypanel{padding-top:46px;}
                }
            <?php } ?>
            .lms-topic-sidebar-wrapper,#learndash-page-content{min-height:calc(100vh - <?php echo $header_sum - 5; ?>px);}
            @media screen and (max-width: 782px) {
                .lms-topic-sidebar-wrapper,#learndash-page-content{min-height:calc(100vh - <?php echo $header_sum - 15; ?>px);}
            }
            <?php
            echo '</style>';
        }
    ...
    add_action('wp_head', 'buddyboss_theme_ld_style', 100);
}
